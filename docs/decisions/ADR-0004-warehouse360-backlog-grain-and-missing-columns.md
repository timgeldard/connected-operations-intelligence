...
> SQL, no source mode, no contract promotion. It records the recommended target Gold/contract changes so
> the implementation can proceed in scoped follow-up PRs. It does **not** make Warehouse360 governed mode
> ready, does **not** prove UAT readiness, and does **not** switch the app source mode.
...
- **Recommended:** reduce the contract to **order grain** for now (drop `reservation_no`/`batch_id` from
  ...
  `material_name ← join silver.material on material_code`, `sap_order ← order_number` (confirm intent).
...
### D4 — `outbound_backlog`: add available columns, join customer dim, decide customer_id, drop carrier
`silver.outbound_delivery` (same delivery grain) HAS `actual_goods_issue_date`, `delivery_date`,
`gross_weight` → **add to `gold_delivery_pick_status`** (`missing-but-available-upstream`).
`customer_name` → **join `silver.customer`** (`customer_code` ↔ `sold_to_customer`)
(`missing-requires-dimension-join`). `customer_id` → **contract-semantic decision**: define `customer_id`
as the **sold-to party** and map `sold_to_customer AS customer_id` *only if ratified*; otherwise keep it
a documented blocker (do not assume the equivalence). `carrier` → **no replicated source**
...
model (gold owner's call). `customer_id`-as-sold-to (D4) requires explicit business ratification.
