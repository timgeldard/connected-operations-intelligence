...
**Silver stage-gate — FIXED and proven (the #58 regression check passes).** The 6 operational tables
checked in this run (a subset of the full ENFORCED set in `silver_stage_gate_inventory.yml`) are now
scoped to exactly the two onboarded plants (C061 + P817) — the 640-plant / 18.8M-row `purchase_order`
leak is gone:
...
**Security / RLS — NOT proven (and NOT a data-shape blocker).** Used `validation_open` — the mode built
...
as the object owner, so the run did not depend on UC security. A separate finding — **the `users` consumer
group does not exist in this UAT metastore** (`PRINCIPAL_DOES_NOT_EXIST` on every `GRANT … TO users` and
the harden `REVOKE … FROM users`) — therefore did **not** block data-shape validation; it only affects the
...
**Outcome A (partial) — the data-shape blocker is now fixed.** The residual `overview` (133) +
`im_wm_reconciliation` (327) leak (ungated `storage_bin`/`stock_at_location`) is **fixed in
`fix/silver-gate-storage-bin-stock-at-location`** and DEV-validated (both views → 2 plants; gold_warehouse_
kpi_snapshot/exceptions → 2 plants). After that fix, a UAT re-run under `validation_open` is expected to
