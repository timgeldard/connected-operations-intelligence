...
| DEV consumption view live validation | Partial — **4/7 create (revalidated 2026-06-08)**: `overview` (DQ caveats), `outbound_backlog` (D4 carrier removed), `stock_exceptions` (D5 storage_loc removed — creates empty, source has no rows), `im_wm_reconciliation` (D6 re-grained to aggregate exception-summary — 198,860 rows, 0 dup PK by construction). Remaining 3 = grain: `po_id` (D1), `material_id` (D2), `reservation_no`+`sap_order` (D3); plus overview DQ (D7). RLS/entitlement still unproven. | [DEV live-validation results](warehouse360-dev-live-validation-results.md) | next: D7 (gold-side overview DQ), or D1/D2 detail-grain Gold models |
