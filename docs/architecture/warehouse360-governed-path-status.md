...
| DEV consumption view live validation | Partial — **6/7 create (revalidated 2026-06-08)**: `overview` (543/0/0 — D7/PR A), `outbound_backlog` (D4), `stock_exceptions` (D5 — empty), `im_wm_reconciliation` (D6 aggregate — 198,860 rows, 0 dup PK), `staging_workload` (D3 order grain — empty), `inbound_backlog` (D1 — new gold_inbound_po_line_backlog PO-line model, 1,112,080 rows, 0 dup PK). Remaining 1 = `material_id` (D2 material-grain shortfalls Gold model). RLS/entitlement still unproven; stock_exceptions + staging are created-empty in the DEV shakedown. | [DEV live-validation results](warehouse360-dev-live-validation-results.md) | next: D2 material-grain shortfalls Gold model, then full revalidation → 7/7 |
