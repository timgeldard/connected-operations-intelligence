...
| UAT validation | **Revalidated 2026-06-09 after #58/#60/#62/#63 — Outcome A (partial).** Full UAT rerun (Silver slow→fast→Gold→validation_open→serving→consumption→contract SQL). **Silver gate FIXED + proven**: the 6 operational tables checked this run (a subset of the ENFORCED set) = C061+P817 only (`purchase_order` 184,553/2 — the 640-plant leak is gone); T320 mapping C061→104, P817→208. All 7 consumption views CREATE. Two views (`overview` 133, `im_wm_reconciliation` 327) still leaked via ungated `storage_bin`/`stock_at_location` — **now fixed in `fix/silver-gate-storage-bin-stock-at-location` (DEV-validated: both → 2 plants)**. 4 other views correct (2 plants); staging/stock_exceptions empty (source gold 0). Deployment torn down. | [UAT validation results](warehouse360-uat-validation-results.md) | re-run UAT under validation_open (leak fix landed) |
