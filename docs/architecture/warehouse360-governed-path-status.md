...
| DEV consumption view live validation | Partial — **2/7 create (revalidated 2026-06-08)**: `overview` (data-quality caveats) + `outbound_backlog` (2,162,748 rows, 0 null plant_id, 0 dup PK after `carrier` removed, ADR-0004 D4). PR #43 moved the frontier; `carrier` first-wave reduction unblocked outbound. Remaining 5 = grain/no-source: `po_id` (D1), `material_id` (D2), `reservation_no` (D3), `storage_location_id`/`bin_id` (D5/D6), + overview DQ (D7). RLS/entitlement still unproven. | [DEV live-validation results](warehouse360-dev-live-validation-results.md) | next: D5/D6 as app-contract migration, or D7 (gold-side), or D1/D2 grain models |
