...
| DEV consumption view live validation | Partial — **5/7 create (revalidated 2026-06-08)**: `overview` (valid shape, 543/0/0 — D7/PR A), `outbound_backlog` (D4 carrier removed), `stock_exceptions` (D5 — creates empty), `im_wm_reconciliation` (D6 aggregate — 198,860 rows, 0 dup PK), `staging_workload` (D3 reduced to order grain — creates empty, gold_process_order_staging 0 rows in DEV). Remaining 2 = new Gold models: `po_id` (D1 inbound PO-line), `material_id` (D2 material-grain shortfalls). RLS/entitlement still unproven; stock_exceptions + staging are created-empty in the DEV shakedown. | [DEV live-validation results](warehouse360-dev-live-validation-results.md) | next: D1 inbound PO-line + D2 material-grain shortfalls Gold models, then full revalidation |
