-- Governed readiness configuration (DEV).
-- WARNING: Generated automatically by scripts/generate_readiness_config_sql.py. Do not edit manually.

-- ── site_config_plant ──
CREATE TABLE IF NOT EXISTS connected_plant_dev.silver_dev.site_config_plant (
  plant_code STRING,
  plant_name STRING,
  country STRING,
  region STRING,
...
BEGIN;
DELETE FROM connected_plant_dev.silver_dev.site_config_plant WHERE config_owner = 'wm-config-owner';
INSERT INTO connected_plant_dev.silver_dev.site_config_plant (plant_code, plant_name, country, region, business_unit, timezone, sap_system_id, go_live_status, wm_enabled_flag, hu_enabled_flag, qm_enabled_flag, spc_enabled_flag, batch_managed_flag, process_manufacturing_flag, default_language_code, valid_from, valid_to, is_active, config_owner, last_validated_at) VALUES
...
CREATE TABLE IF NOT EXISTS connected_plant_dev.silver_dev.site_config_warehouse (
  plant_code STRING,
...
BEGIN;
DELETE FROM connected_plant_dev.silver_dev.site_config_warehouse WHERE config_owner = 'wm-config-owner';
INSERT INTO connected_plant_dev.silver_dev.site_config_warehouse (plant_code, warehouse_number, warehouse_description, relationship_type, wm_usage_type, is_shared_warehouse, valid_from, valid_to, is_active, config_owner) VALUES
...
CREATE TABLE IF NOT EXISTS connected_plant_dev.silver_dev.site_config_storage_type_role (
  plant_code STRING,
  warehouse_number STRING,
  storage_type STRING,
...
BEGIN;
DELETE FROM connected_plant_dev.silver_dev.site_config_storage_type_role WHERE validated_by = 'wm-config-owner';
INSERT INTO connected_plant_dev.silver_dev.site_config_storage_type_role (plant_code, warehouse_number, storage_type, storage_type_description, storage_role, role_confidence, is_wm_managed, include_in_lineside_stock, include_in_staging, include_in_reconciliation, valid_from, valid_to, validated_by, validated_at) VALUES
...
CREATE TABLE IF NOT EXISTS connected_plant_dev.silver_dev.site_config_movement_type_classification (
  plant_code STRING,
...
BEGIN;
DELETE FROM connected_plant_dev.silver_dev.site_config_movement_type_classification WHERE plant_code = 'C061' OR plant_code IS NULL;
INSERT INTO connected_plant_dev.silver_dev.site_config_movement_type_classification (plant_code, movement_type_code, movement_text, event_category, is_production_receipt, is_production_consumption, is_scrap, is_reversal, reversal_of_movement_type, is_inbound_receipt, is_outbound_issue, is_stock_adjustment, classification_source, validation_status, valid_from, valid_to) VALUES
...
CREATE TABLE IF NOT EXISTS connected_plant_dev.silver_dev.site_config_staging_method (
  plant_code STRING,
  warehouse_number STRING,
...
BEGIN;
DELETE FROM connected_plant_dev.silver_dev.site_config_staging_method WHERE plant_code = 'C061' OR plant_code IS NULL;
INSERT INTO connected_plant_dev.silver_dev.site_config_staging_method (plant_code, warehouse_number, production_supply_area, storage_type, staging_method, sap_reference_pattern, requires_batch_scan, requires_sscc, validation_status, valid_from, valid_to) VALUES
...
CREATE TABLE IF NOT EXISTS connected_plant_dev.silver_dev.site_config_kpi_enablement (
  plant_code STRING,
...
BEGIN;
DELETE FROM connected_plant_dev.silver_dev.site_config_kpi_enablement WHERE approved_by = 'wm-config-owner';
INSERT INTO connected_plant_dev.silver_dev.site_config_kpi_enablement (plant_code, data_product_name, kpi_name, enablement_status, reason_code, approved_by, approved_at, review_due_at) VALUES
