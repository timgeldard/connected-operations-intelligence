...
SELECT
  ...
  net_adjustment_qty
FROM connected_plant_dev.gold_io_reporting.gold_wm_pi_accuracy_secured
WHERE plant_code IS NOT NULL;
...
-- ⚠️ Data scope ≤ 2023-12-05 in DEV bronze (UAT-snapshot artefact). Prod data is current.
...
SELECT
  ...
  CASE
    ...
    ELSE NULL
  END                                                    AS days_overdue
FROM connected_plant_dev.gold_io_reporting.gold_wm_push_despatch_delivery_secured d
...
SELECT
  ...
  agg.on_time_pgi_pct
FROM connected_plant_dev.gold_io_reporting.gold_wm_push_despatch_daily_secured agg
