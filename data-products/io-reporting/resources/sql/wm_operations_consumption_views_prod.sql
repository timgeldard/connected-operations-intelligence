...
FROM connected_plant_prod.gold_io_reporting.gold_wm_expiry_risk_secured
WHERE plant_code IS NOT NULL;
...
SELECT
  ...
  ud_lead_time_days,
  is_overdue
FROM connected_plant_prod.gold_io_reporting.gold_wm_qm_lot_status_live
WHERE plant_code IS NOT NULL;
...
FROM connected_plant_prod.gold_io_reporting.vw_consumption_wm_operations_worklist_summary
...
FROM connected_plant_prod.gold_io_reporting.gold_wm_order_journey_summary_secured
WHERE plant_code IS NOT NULL;
...
SELECT
  ...
  reference_id,
  detail
FROM connected_plant_prod.gold_io_reporting.gold_wm_order_journey_events_secured
WHERE plant_code IS NOT NULL;
...
SELECT
  ...
  CAST(first_gr_posting_date AS DATE) AS first_gr_posting_date,
  gr_qty
FROM connected_plant_prod.gold_io_reporting.gold_wm_order_wip_stage_secured
WHERE plant_code IS NOT NULL;
...
FROM connected_plant_prod.gold_io_reporting.gold_process_order_schedule_adherence_secured
WHERE plant_code IS NOT NULL
...
SELECT
  ...
  sort_seq,
  base_uom AS uom
FROM connected_plant_prod.gold_io_reporting.gold_wm_supply_demand_ledger_secured
WHERE plant_code IS NOT NULL;
...
SELECT
  ...
  CAST(scheduled_finish_date AS DATE) AS scheduled_finish_date,
  production_line
FROM connected_plant_prod.gold_io_reporting.gold_wm_order_shortage_projection_secured
WHERE plant_code IS NOT NULL;
...
SELECT
  ...
  net_adjustment_qty
FROM connected_plant_prod.gold_io_reporting.gold_wm_pi_accuracy_secured
WHERE plant_code IS NOT NULL;
...
FROM connected_plant_prod.gold_io_reporting.gold_wm_lineside_lines_secured
WHERE plant_code IS NOT NULL;
...
SELECT
  ...
  y.is_completed,
  y.is_closed
FROM connected_plant_prod.gold_io_reporting.gold_wm_order_journey_summary_secured j
INNER JOIN connected_plant_prod.gold_io_reporting.gold_wm_order_yield_secured y
  ON j.order_number = y.order_number
  AND j.plant_code  = y.plant_code
LEFT JOIN connected_plant_prod.gold_io_reporting.gold_wm_lineside_now_live ln
  ON j.order_number = ln.order_number
  AND j.plant_code  = ln.plant_code
LEFT JOIN connected_plant_prod.gold_io_reporting.gold_wm_adherence_root_cause_secured arc
...
LEFT JOIN connected_plant_prod.gold_io_reporting.gold_wm_order_readiness_secured r
