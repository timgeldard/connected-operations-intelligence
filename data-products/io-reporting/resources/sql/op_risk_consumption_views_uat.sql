-- Operational Risk Consumption Views (UAT)
...
WITH base_items AS (
  SELECT
    ...
    food_safety_flag
  FROM connected_plant_uat.gold_io_reporting.gold_operational_risk_item_secured
...
freshness_critical AS (
  ...
  FROM connected_plant_uat.gold_io_reporting.gold_domain_freshness_watermark_secured
...
GRANT SELECT ON VIEW connected_plant_uat.gold_io_reporting.vw_consumption_op_risk_operational_risk_live TO `users`;
...
GRANT SELECT ON VIEW connected_plant_uat.gold_io_reporting.vw_consumption_data_freshness TO `users`;
