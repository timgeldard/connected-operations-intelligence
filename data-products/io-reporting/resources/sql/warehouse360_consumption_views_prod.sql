-- Warehouse360 Consumption Views (PROD)
-- Target: connected_plant_prod.gold_io_reporting
-- TODO_PROD_CATALOG: replace connected_plant_prod with approved production catalog.
...
FROM connected_plant_prod.gold_io_reporting.gold_warehouse_kpi_snapshot_live
...
FROM connected_plant_prod.gold_io_reporting.gold_inbound_po_line_backlog_live;
...
FROM connected_plant_prod.gold_io_reporting.gold_transfer_requirement_material_backlog_secured;
...
SELECT
  ...
  confirmed_by_user,
  age_hours
FROM connected_plant_prod.gold_io_reporting.gold_transfer_order_open_items_live;
...
SELECT
  ...
  order_reference_number,
  age_hours
FROM connected_plant_prod.gold_io_reporting.gold_transfer_requirement_open_items_live;
...
SELECT
  ...
  transaction_code
FROM connected_plant_prod.gold_io_reporting.gold_goods_movement_activity_secured;
