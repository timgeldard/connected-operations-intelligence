...
CREATE TABLE IF NOT EXISTS connected_plant_uat.gold_io_reporting.spc_exclusions
  DEEP CLONE connected_plant_uat.gold.spc_exclusions;

-- ── spc_mic_chart_config ───────────────────────────────────────────────────────────────

-- No bridge view exists for this table: clone directly to the target name (fully idempotent; no orphaned _migrated tables).
CREATE TABLE IF NOT EXISTS connected_plant_uat.gold_io_reporting.spc_mic_chart_config
  DEEP CLONE connected_plant_uat.gold.spc_mic_chart_config;

-- ── spc_query_audit ───────────────────────────────────────────────────────────────

-- No bridge view exists for this table: clone directly to the target name (fully idempotent; no orphaned _migrated tables).
CREATE TABLE IF NOT EXISTS connected_plant_uat.gold_io_reporting.spc_query_audit
