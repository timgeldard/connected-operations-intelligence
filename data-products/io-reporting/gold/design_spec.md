...
| `gold_stock_holds` | plant × wh × storage_type × bin × quant | hold_type (quality/blocked/restricted), quantity, goods_receipt_date | occupied WM quants under hold (LQUA category Q/S, or batch with MCHB restricted-use stock); `age_hours` is query-time in `_live`; feeds Warehouse360 open-holds; hold provenance/raised-by is a data gap (no QM hold log replicated) — fast tier |
