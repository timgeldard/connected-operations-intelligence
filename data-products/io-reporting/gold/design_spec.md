...
| `gold_stock_holds` | plant × wh × storage_type × bin × quant | hold_type (quality/blocked/restricted), quantity, goods_receipt_date | occupied WM quants under hold (LQUA category Q/S, or batch with MCHB restricted-use stock — restricted is BATCH-LEVEL by design: LQUA lacks LGORT so sloc-grain attribution is impossible); `age_hours` is query-time in `_live`; feeds Warehouse360 open-holds; hold provenance/raised-by is a data gap (no QM hold log replicated) — fast tier |
| `gold_transfer_order_open_items` | wh × TO × item | requested/confirmed qty, item_status, source/dest ST+bin, BENUM/BETYP, users | open TO items (`item_status != 'Fully Confirmed'`, matching kpi-snapshot/exceptions semantics) with header-delete anti-join; `age_hours` query-time in `_live`; feeds Warehouse360 pick-tasks — fast tier |
| `gold_transfer_requirement_open_items` | wh × TR × item | required/open qty, source/dest ST+bin, queue, priority, BENUM/BETYP | open TR items (not processing-complete, open_quantity>0); `age_hours` query-time in `_live`; feeds Warehouse360 move-requests; assignee is a data gap (LTBK has none) — fast tier |
| `gold_goods_movement_activity` | MSEG line (doc × year × line) | quantity/value, movement flags (receipt/issue/transfer/reversal), references (PO/order/delivery), posted_by | IM postings + movement-type classification; HIGH-VOLUME — clustered on plant_code × posting_date; route enforces default 1-day window, hard 31-day max, limit ≤ 500; UAT perf/cost validation required before frontend ships — fast tier |
