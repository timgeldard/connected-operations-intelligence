...
CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_order_downtime_summary_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_order_downtime_summary
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_order_downtime_summary_secured TO `users`;

CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_process_order_component_status_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_process_order_component_status
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_process_order_component_status_secured TO `users`;
...
CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_stock_value_reconciliation_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_stock_value_reconciliation
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_stock_value_reconciliation_secured TO `users`;

CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_reconciliation_audit_log_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_reconciliation_audit_log
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_reconciliation_audit_log_secured TO `users`;

CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_movement_reconciliation_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_movement_reconciliation
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_movement_reconciliation_secured TO `users`;

CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_hu_reconciliation_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_hu_reconciliation
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_hu_reconciliation_secured TO `users`;

CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_physical_inventory_recon_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_physical_inventory_recon
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_physical_inventory_recon_secured TO `users`;

CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_reconciliation_alerts_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_reconciliation_alerts
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_reconciliation_alerts_secured TO `users`;
...
CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_stock_reconciliation_summary_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_stock_reconciliation_summary
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_stock_reconciliation_summary_secured TO `users`;
...
CREATE OR REPLACE VIEW connected_plant_uat.gold.gold_inbound_po_backlog_enhanced_secured AS
  SELECT * FROM connected_plant_uat.gold.gold_inbound_po_backlog_enhanced
  WHERE connected_plant_uat.silver.plant_access_filter(plant_code);
GRANT SELECT ON VIEW connected_plant_uat.gold.gold_inbound_po_backlog_enhanced_secured TO `users`;
